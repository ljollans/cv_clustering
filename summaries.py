...
from cv_clustering.mainfoldaggr import agglom, aggr4comp, agglom_best_k_per_sf, doagglomchks, agglomerrorwrap, \
    cross_sf_similarity_chk, samekagglom_error_mf, moreagglomloop, get_labels_rand, get_labels_rand_cert
...
for ii in [2]:
    ...
    if do_level_30==1:
        ...
        averageeuc = np.full([12, 4, n_k, n_k], np.nan)
        ...
        mse_btw_vecs = np.full([12, 4, n_k, n_k, n_k + 2], np.nan)
        avgclus=[None]*len(sets)
        for s in range(len(sets)):
            filepath2use = (input_filedir + sets[s] + modstr)
            avgclus[s]=[None]*n_cv_folds
            for mf in range(n_cv_folds):
                averageerror[s,mf,:,:],averageeuc[s,mf,:,:], n_vecs[s,mf,:,:,:], source_vecs[s,mf,:,:,:], mse_btw_vecs[s,mf,:,:,:],ed_btw_vecs[s,mf,:,:,:], avgclus[s][mf] = doagglom_moremet(0, mf ,n_cv_folds, setsize[s], n_k, filepath2use)
                #averageerror,averageeuc, n_vecs, source_vecs, mse_btw_vecs,ed_btw_vecs, avgclus

        with open((input_filedir + 'agglom_moremet.pkl'), 'wb') as f:
            pickle.dump([averageerror,averageeuc,n_vecs,source_vecs,mse_btw_vecs,ed_btw_vecs,avgclus], f)

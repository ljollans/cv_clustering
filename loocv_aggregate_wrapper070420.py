...
from loocv_assigmatcher_nov import pull_from_mod
...
path2use_2='/Users/lee_jollans/Projects/clustering_pilot/FEB_PUT/newsav_'
...
for current_set in [0]:#range(len(sets)):
    ...
    for ctr in [1]: #range(2):
        ...
        for mainfold in [0]: #range(4):
            for subfold in [0]: # range(4):
                ...
                tmp_pac, tmp_rand_all, tmp_coph, tmp_aggrbetas, tmp_preaggrbetas, tmp_conlab = pull_from_mod(mod)
                s_PAC.append(tmp_pac)
                s_RAND.append(tmp_rand_all)

...
def calculate_clustering_metrics(design):
    ...
    return clus_labels, bic, sil, cal, np.mean(auc_across_cv_folds), np.mean(f1_across_cv_folds), beta_avg_across_folds, n_across_cv_folds

def calculate_clustering_metrics_spectral(design):
    spec=cluster.SpectralClustering(
        n_clusters=design["nclus"] + 2,
        eigen_solver='arpack',
    ...
    clus_labels = spec.labels_.astype(np.int)
    bic = np.nan#gmm.bic(design["data"])
    sil = metrics.silhouette_score(design["data"], clus_labels)
    cal = metrics.calinski_harabasz_score(design["data"], clus_labels)
